...
const FormComponent = ({ onSubmit, task, currentUserEmail }) => {
    ...
    const [loadingPersons, setLoadingPersons] = true);
    ...
    */
    // Dependency on dailyHours to preserve existing values
                ...
                try {
                    ...
                    if (data && data.entries) {
                        data.entries.forEach(entry => {
                            ...
                            existingDailyHours[moment(entry.Day).format('YYYY-MM-DD')] 
                            = Math.round((entry.Duration || 0) * 60); // Round to nearest minute
                        });
                    }
    ...
    }, [task]); // Removed calculateEndDate dependency
    // Dependencies: task and calculateEndDate
    ...
    */
    ...
    const handleDailyHoursSliderChange = (date) => (e) => {
        const value = parseInt(e.target.value, 10);
        // Value from slider is in minutes
    ...
    */
    ...
    const handleSubmit = async (e) => {
        ...
        setSuccess(null);
        // Basic validation for required fields: only check for Responsibility
        ...
        try {
            ...
            const mainTaskPayload = {
                ...
                // Convert 
                // moment objects to ISO strings for backend
                ...
                null, // Preserve existing or set null
            ...
            for (let i = 0; i < formData.Number_of_Days; i++) {
                const currentDayMoment = formData.Planned_Start_Timestamp.clone().add(i, 'days');
                const formattedDay = currentDayMoment.format('YYYY-MM-DD');
                if (dailyHours[formattedDay] !== undefined) {
                    perKeyPerDayRows.push({
                        Key: mainTaskPayload.Key,
                        Day: formattedDay, // Ensure this is the correct date string

              
                        Duration: dailyHours[formattedDay], // Send minutes directly
                        Duration_Unit: 'min', // Explicitly set to 'min' as per backend expectation
            ...
            */
            ...
            const payload = {
                ...
                perKeyPerDayRows: perKeyPerDayRows // Will be an empty array
    ...
    // Already defined globally if needed
            ...
            */}
            ...
            <Form.Group className="mb-3">
                ...
                <Select
                    ...
                    isDisabled={!isAdmin ||
                    loadingPersons || isFieldDisabledForNonAdmin}
            ...
            isFieldDisabledForNonAdmin}>

                {loading ?
                <Spinner as="span" animation="border" size="sm" role="status" aria-hidden="true" /> : 'Update Task'}
