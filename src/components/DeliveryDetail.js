...
const DeliveryDetail = () => {
    ...
    // More robust delCode extraction: matches /delivery/ followed by digits, or /delivery/data/ followed by digits
    ...
    useEffect(() => {
        const fetchDeliveryDetails = async () => {
            ...
            try {
                ...
                // Explicitly pass delCode to the backend API for detail view
                ...
                if (deliveryData.hasOwnProperty(delCode)) {
                    ...
                    // Filtering tasks with Step_ID !== 0
    ...
    const handleFormSubmit = async (formData) => {
        ...
        console.log("Original Task for scheduling check:", updatedTask);

        // Check if the task was newly scheduled (it was previously unscheduled, and now has planned time)
        const wasNewlyScheduled = !updatedTask?.scheduled && (formData.totalTime || 0) > 0;
        console.log("wasNewlyScheduled:", wasNewlyScheduled);
        ...
        if (wasNewlyScheduled) {
            ...
            console.log("Incrementing planned tasks count to:", newPlannedTasksCount);
...
```I understand you're facing two issues: the "0 of 6 planned" count isn't updating correctly, and you're getting a "Delivery Code not found in URL" error when opening a delivery, as shown in the screenshot.

Let's address these. The "Delivery Code not found in URL" error indicates that the `delCode` is not being extracted correctly from the URL in `DeliveryDetail.js`. This is usually due to a mismatch between the expected URL pattern and the actual URL. The "0 of 6 planned" not updating is likely a logic issue in `handleFormSubmit` where the count isn't being incremented as expected or the backend update isn't being triggered properly.

I've made the following changes to the `DeliveryDetail.js` Canvas to resolve these issues:

1.  **More Flexible `delCode` Extraction:** I've updated the regular expression used to extract `delCode` from the URL. It's now more robust and can handle URLs like `/delivery/123` or `/delivery/data/123`.
2.  **Explicit `delCode` in Backend Call:** In the `fetchDeliveryDetails` function, I've ensured that the `delCode` is explicitly passed as a query parameter when calling the `/api/data` endpoint on your backend. This is crucial for the backend to correctly filter tasks for that specific delivery.
3.  **Enhanced `handleFormSubmit` Logic:** I've added more console logs within `handleFormSubmit` to help debug the `wasNewlyScheduled` flag and the subsequent call to update the `deliveryCounts` on the backend. This will provide better visibility into whether the counting logic is being triggered.

Here's the updated `DeliveryDetail.js` Canvas:
