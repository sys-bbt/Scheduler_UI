                ...
                <Card.Body>
                    ...
                    {isTaskScheduled && !isTaskFinished && (
                        ...
                        <div className='d-flex justify-content-between mt-3' onClick={(e) => e.stopPropagation()}>
                            ...
                            <Button 
                                ...
                                onClick={(e) => handleStatusUpdate(e, 'Complete')}
                            >
                                <FaCheckCircle size={20} />
                            </Button>

                            {/* NOT REQUIRED Button - Admin only CONDITIONAL RENDERING */}
                            {isAdmin && (
                                <Button 
                                    variant="secondary" 
                                    className="w-50 ms-2 d-flex align-items-center justify-content-center"
                                    title="Mark Not Required (Admin)"
                                    onClick={(e) => handleStatusUpdate(e, 'Not Required')}
                                >
                                    <FaTimesCircle size={20} />
                                </Button>
                            )}
                            
                            {/* If not admin, use an empty placeholder to keep the layout clean if necessary, or just render the Complete button full width */}
                            {!isAdmin && (
                                <Button 
                                    variant="success" 
                                    className="w-100" // Make Complete button full width if Not Required button isn't shown
                                    title="Mark Complete" 
                                    onClick={(e) => handleStatusUpdate(e, 'Complete')}
                                >
                    ...
                    {isActive && (
                        <div className="mt-3" onClick={(e) => e.stopPropagation()}> 
                            {/* Removed redundant <h6> since FormComponent has its own title */}
                            <FormComponent
